...
import { z } from 'zod'; 
import * as JsonRepairLib from 'jsonrepair';
 ...
 * Uses recursive regex to find the outermost curly brace structure as provided by the user.
 */
...
const safeParseAIResponse = <TData>(text: string, schema: z.ZodSchema<TData>): TData | null => {
  ...
  try {
    ...
    try {
      ...
      const repairFn = (JsonRepairLib as any).jsonrepair || (JsonRepairLib as any).default || JsonRepairLib;
      const repaired = repairFn(jsonCandidate);
