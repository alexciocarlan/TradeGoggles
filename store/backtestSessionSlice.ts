...
export const createBacktestSessionSlice: StateCreator<AppState, [], [], BacktestSessionSlice> = (set, get) => ({
  ...
  saveBacktestDailyPrep: async (sessionId: string, date: string, prep: Partial<DailyPrepData>) => {
    let updatedSession: BacktestSession | undefined;
    
    set(produce((state: AppState) => {
      const s = state.backtestSessions.find(item => item.id === sessionId);
      if (s) {
        if (!s.simulatedDailyPreps) s.simulatedDailyPreps = {};
        s.simulatedDailyPreps[date] = { ...(s.simulatedDailyPreps[date] || {}), ...prep } as DailyPrepData;
        updatedSession = s;
      }
    }));
  ...
  saveBacktestWeeklyPrep: async (sessionId: string, weekId: string, prep: WeeklyPrepData) => {
    ...
    set(produce((state: AppState) => {
      const s = state.backtestSessions.find(item => item.id === sessionId);
      if (s) {
        if (!s.simulatedWeeklyPreps) s.simulatedWeeklyPreps = {};
        s.simulatedWeeklyPreps[weekId] = prep;
        updatedSession = s;
      }
    }));
