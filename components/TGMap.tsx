...
import { useAppStore } from '../AppContext'; // FIX: Changed useAppContext to useAppStore
...
const TGMap: React.FC<{ onOpenScoreModal: () => void }> = ({ onOpenScoreModal }) => {
  const navigate = useNavigate();
  // FIX: Destructure trades and selectedAccountId from useAppStore
  const { dailyPreps, trades, selectedAccountId } = useAppStore();
  
  // FIX: Re-implement activeTrades as a useMemo calculation
