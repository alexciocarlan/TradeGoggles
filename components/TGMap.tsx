...
const TGMap: React.FC = () => {
  const navigate = useNavigate();
  const { dailyPreps, activeTrades } = useAppContext();
  ...
  return (
    <div className="relative w-full h-[calc(100vh-120px)] rounded-[3.5rem] overflow-hidden border border-slate-800 shadow-2xl animate-in fade-in duration-1000 bg-[#03070c]">
      ...
      {/* ELITE TARGET INDICATOR - MODIFIED TO BE CLICKABLE */}
      <div 
      ...
      {/* FULL SCREEN MODAL / SHEET FOR TG SCORE */}
      {isScoreModalOpen && (
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-6 bg-black/95 backdrop-blur-3xl animate-in fade-in duration-500">
          <div className="bg-[#0b1222] border border-blue-500/30 rounded-[3rem] w-full max-w-3xl shadow-[0_0_120px_rgba(37,99,235,0.2)] overflow-hidden flex flex-col relative animate-in zoom-in-95 duration-500">
             {/* Header */}
             <div className="p-10 border-b border-slate-800/80 bg-slate-900/10 flex justify-between items-center">
                <div className="flex items-center space-x-6">
                  <div className="w-14 h-14 rounded-2xl bg-blue-600 flex items-center justify-center text-white shadow-[0_0_30px_rgba(37,99,235,0.4)]">
                    <i className="fas fa-microchip text-2xl"></i>
                  </div>
                  <div>
                    <p className="text-[10px] font-black text-blue-500 uppercase tracking-[0.4em] mb-1">Live Identity Sync</p>
                    <h3 className="text-3xl font-black text-white uppercase italic tracking-tighter leading-none">SENTINEL SCORE ANALYTICS</h3>
                  </div>
                </div>
                <button onClick={() => setIsScoreModalOpen(false)} className="w-12 h-12 rounded-xl bg-slate-900 border border-slate-800 flex items-center justify-center text-slate-500 hover:text-white transition-all">
             ...
             <div className="p-10 flex flex-col items-center">
                <div className="w-full h-80 relative mb-10">
                   ...
                   <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                      <span className="text-6xl font-black text-white italic tracking-tighter drop-shadow-[0_0_20px_rgba(255,255,255,0.1)]">{Math.round(scoreData.score)}</span>
                      <span className="text-[10px] font-black text-blue-500 uppercase tracking-widest">Global Status</span>
                   </div>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full mb-10">
                    <div className="bg-slate-950/40 p-6 rounded-3xl border border-slate-800 text-center shadow-inner">
                        <p className="text-[8px] font-black text-slate-600 uppercase tracking-widest mb-2">Preparation</p>
                        <p className="text-2xl font-black text-white italic">{Math.round(scoreData.components[0]?.A || 0)}%</p>
                    </div>
                    <div className="bg-slate-950/40 p-6 rounded-3xl border border-slate-800 text-center shadow-inner">
                        <p className="text-[8px] font-black text-slate-600 uppercase tracking-widest mb-2">Execution</p>
                        <p className="text-2xl font-black text-white italic">{Math.round(scoreData.components[1]?.A || 0)}%</p>
                    </div>
                    <div className="bg-slate-950/40 p-6 rounded-3xl border border-slate-800 text-center shadow-inner">
                        <p className="text-[8px] font-black text-slate-600 uppercase tracking-widest mb-2">Review</p>
                        <p className="text-2xl font-black text-white italic">{Math.round(scoreData.components[2]?.A || 0)}%</p>
                    </div>
                </div>

                {scoreData.vetoTriggered && (
                  <div className="w-full bg-red-600/10 border border-red-500/40 p-8 rounded-[2rem] flex items-center space-x-6 animate-pulse">
                      <div className="w-12 h-12 rounded-full bg-red-600 flex items-center justify-center text-white shrink-0 shadow-lg shadow-red-600/20">
                        <i className="fas fa-ban text-xl"></i>
                      </div>
                      <div>
                        <h4 className="text-red-500 font-black uppercase text-sm tracking-widest mb-1">SENTINEL VETO DETECTED</h4>
                        <p className="text-xs text-red-400 font-bold uppercase italic leading-none">{scoreData.vetoReason}: Score penalty active.</p>
                      </div>
                  </div>
                )}
             </div>

             <div className="p-10 border-t border-slate-800/80 bg-slate-950/20 flex justify-center">
                <button 
                  onClick={() => setIsScoreModalOpen(false)}
                  className="px-16 py-5 bg-blue-600 hover:bg-blue-500 text-white font-black text-xs uppercase tracking-[0.4em] rounded-3xl transition-all shadow-2xl shadow-blue-600/30 active:scale-95"
                >
                  RETURN TO MAP
                </button>
             </div>
             
             <i className="fas fa-shield-halved absolute -bottom-20 -right-20 text-[300px] text-white/[0.01] pointer-events-none rotate-12"></i>
          </div>
        </div>
      )}
