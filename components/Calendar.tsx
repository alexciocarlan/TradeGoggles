...
// Removed redundant Language import (already in types.ts)
import { useAppStore } from '../AppContext';

const Calendar: React.FC = () => {
  // FIX: Asigurat fallback pentru trades [] direct în selector
  const trades = useAppStore(state => state.trades || []);
  const dailyNotes = useAppStore(state => state.dailyNotes);
  const saveDailyNote = useAppStore(state => state.saveDailyNote);
  const deleteTrade = useAppStore(state => state.deleteTrade);
  ...
  // NEW: Get lazy load functions
  const loadDailyPreps = useAppStore(state => state.loadDailyPreps);
  const loadDailyNotes = useAppStore(state => state.loadDailyNotes);
  ...
  // NEW: Load daily preps and notes when month changes
  useEffect(() => {
