  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 pb-20 px-4">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 no-print">
        <div>
          <h3 className="text-3xl font-black uppercase tracking-tight text-white italic">PORTFOLIO MANAGEMENT</h3>
          <p className="text-slate-500 text-[11px] font-black uppercase tracking-widest mt-1.5">REAL-TIME APEX GATEWAY STATUS // NODE: ORD-CHICA-02</p>
        </div>
        <button 
          onClick={onAdd}
          className="bg-blue-600 hover:bg-blue-500 text-white px-8 py-4 rounded-2xl font-black text-[11px] uppercase tracking-widest transition-all shadow-2xl shadow-blue-600/30 active:scale-95 flex items-center"
        >
          <i className="fas fa-plus mr-3"></i> ADĂUGARE CONT NOU
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {accounts.map(acc => {
          ...
          return (
            <div key={acc.id} className={`bg-[#0b1222] border rounded-[2.5rem] overflow-hidden transition-all duration-500 shadow-2xl group flex flex-col ${acc.isPA ? 'border-orange-500/30' : 'border-slate-800 hover:border-blue-500/30'}`}>
              {editingId === acc.id ? (
                <div className="p-8 space-y-6">
                   <input type="text" className="w-full bg-slate-900 border border-slate-800 rounded-2xl px-5 py-4 text-sm text-white focus:ring-1 focus:ring-blue-500 outline-none" value={editForm.name} onChange={e => setEditForm({...editForm, name: e.target.value})} />
                ...
                <>
                  <div className={`px-10 py-8 flex justify-between items-start ${acc.isPA ? 'bg-orange-500/5' : 'bg-blue-600/5'}`}>
                    <div>
                      <div className="flex items-center space-x-3 mb-3">
                        <span className={`text-[8px] font-black px-3 py-1 rounded-lg uppercase shadow-sm ${acc.isPA ? 'bg-orange-600 text-white' : 'bg-blue-600 text-white'}`}>
                            {acc.isPA ? 'PERFORMANCE (PA)' : 'EVALUATION'}
                        </span>
                        <span className="text-[8px] font-black text-slate-500 uppercase tracking-widest">TYPE: {acc.type.toUpperCase()}</span>
                      </div>
                      <h4 className="text-2xl font-black text-white uppercase tracking-tighter italic">{acc.name}</h4>
                    </div>
                    <div className="flex space-x-3 opacity-20 group-hover:opacity-100 transition-all">
                      <button onClick={() => startEditing(acc)} className="text-slate-400 hover:text-white p-2.5 bg-slate-900/50 rounded-xl border border-slate-800 transition-all"><i className="fas fa-edit text-xs"></i></button>
                      <button onClick={() => onDelete(acc.id)} className="text-slate-400 hover:text-red-500 p-2.5 bg-slate-900/50 rounded-xl border border-slate-800 transition-all"><i className="fas fa-trash text-xs"></i></button>
                    </div>
                  </div>
                  ...
                  <div className="p-10 pt-4 space-y-8">
                    <div className="grid grid-cols-2 gap-8 border-b border-slate-800/50 pb-8">
                      <div>
                        <p className="text-[10px] font-black text-slate-600 uppercase tracking-widest mb-2 opacity-80">CURRENT BALANCE</p>
                        <p className="text-2xl font-black text-slate-100 tracking-tight">${currentBal.toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-[10px] font-black text-slate-600 uppercase tracking-widest mb-2 opacity-80">LIQUIDATION THRESHOLD</p>
                        <p className="text-2xl font-black text-red-500 tracking-tight">${(acc.initialBalance - acc.maxDrawdown).toLocaleString()}</p>
                      </div>
                    </div>
                    
                    <button 
                      onClick={() => setSelectedAccForOrders(acc)}
                      className="w-full bg-[#0d1629] hover:bg-[#131d36] border border-slate-800 rounded-3xl py-5 flex items-center justify-center space-x-4 transition-all group/btn shadow-inner active:scale-[0.98]"
                    >
                       <div className={`w-2.5 h-2.5 rounded-full ${accOrders.length > 0 ? 'bg-orange-500 animate-pulse shadow-[0_0_15px_rgba(249,115,22,0.6)]' : 'bg-slate-700'}`}></div>
                       <span className="text-[11px] font-black text-slate-400 group-hover/btn:text-white uppercase tracking-[0.2em]">
                         {accOrders.length} ACTIVE ORDERS SYNCED
                       </span>
                    </button>
                    ...
                    <div className="pt-2 flex items-center justify-between">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-[9px] text-slate-600 font-black uppercase tracking-widest">GATEWAY: CHICAGO_PROD_1</span>
                      </div>
                      {acc.isRithmicConnected && (
                          <div className="flex items-center space-x-2 bg-emerald-500/10 px-3 py-1 rounded-lg border border-emerald-500/20">
                              <i className="fas fa-bolt text-emerald-500 animate-pulse text-[9px]"></i>
        ...
        {/* Buton gol pentru a încuraja adăugarea dacă există doar unul */}
        {accounts.length === 1 && (
            <div 
                onClick={onAdd}
                className="border-4 border-dashed border-slate-800/40 rounded-[2.5rem] flex flex-col items-center justify-center p-12 hover:border-blue-500/20 hover:bg-blue-600/[0.02] cursor-pointer transition-all group"
            >
                <div className="w-16 h-16 rounded-full bg-slate-900 flex items-center justify-center mb-6 group-hover:scale-110 transition-transform">
                    <i className="fas fa-plus text-slate-700 group-hover:text-blue-500 text-2xl"></i>
                </div>
                <p className="text-xs font-black text-slate-700 uppercase tracking-[0.3em] group-hover:text-slate-500">Adaugă al doilea cont</p>
            </div>
        )}
      </div>
