...
const Playbooks: React.FC<PlaybooksProps> = ({ playbooks, trades, backtestTrades, language }) => {
  ...
  const { restoreDefaultPlaybooks } = useAppContext();
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700 pb-20">
      <div className="flex flex-col xl:flex-row justify-between items-start gap-6">
        <div>
           <div className="flex items-center space-x-4 mb-2">
                <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em]">SISTEMUL TĂU DE EXECUȚIE (39 TOTAL STRATEGIES)</p>
                <button 
                    onClick={() => { if(confirm('Ești sigur că vrei să resetezi lista la cele 39 de setup-uri implicite?')) restoreDefaultPlaybooks(); }}
                    className="text-[8px] font-black text-red-500/60 hover:text-red-400 uppercase tracking-widest border border-red-500/20 px-2 py-0.5 rounded transition-all"
                >
                    <i className="fas fa-sync-alt mr-1"></i> Restore Defaults
                </button>
           </div>
        ...
        <div className="flex flex-col md:flex-row items-center gap-4 w-full xl:w-auto no-print">
           ...
           <div className="flex items-center space-x-4">
              ...
              <button 
                onClick={handlePrint}
                className="bg-slate-800 hover:bg-slate-700 text-white px-5 py-3.5 rounded-xl font-black text-[10px] transition-all shadow-xl active:scale-95 flex items-center uppercase tracking-widest whitespace-nowrap"
              >
                <i className="fas fa-file-pdf mr-2"></i> Export Vault
              </button>
      ...
      <div className="bg-[#0b1222]/80 border border-slate-800/60 rounded-[2.5rem] overflow-hidden shadow-2xl no-print">
