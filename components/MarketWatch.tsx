...
import { getMarketWatchIntel } from '../geminiService';
...
const MarketWatch: React.FC<MarketWatchProps> = ({ language }) => {
  ...
  const fetchIntel = async (isManual = false) => {
    setLoading(true);
    try {
      const data = await getMarketWatchIntel();
  ...
  return (
    <div className="space-y-8 pb-20 animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
           <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-2">LIVE MARKET SCANNER</p>
           <h2 className="text-3xl font-black text-white uppercase italic tracking-tighter">MARKET WATCH</h2>
           {lastSync && (
             <p className="text-[9px] font-bold text-blue-500/60 uppercase tracking-widest mt-1">
               <i className="fas fa-sync-alt mr-1"></i> Last Sync: {lastSync.toLocaleTimeString()}
             </p>
           )}
        </div>
        <div className="flex items-center space-x-3">
            <button 
                ...
                disabled={loading}
                className="bg-blue-600/10 hover:bg-blue-600/20 border border-blue-500/20 text-blue-400 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center"
            >
      ...
      <div className="bg-blue-600/5 border border-blue-500/20 p-8 rounded-[2.5rem] flex items-start space-x-6 relative overflow-hidden group">
        ...
        <div className="relative z-10">
            ...
            <p className="text-slate-400 text-sm leading-relaxed font-medium">
               ...
               în timp ce structura Yield Curve reflectă faza de ${intel.yieldCurve?.status || 'stabilitate'} economică.` 
