...
import { BacktestSession, Playbook, Account } from '../types';
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          ...
          <div>
            ...
            {backtestAccounts.length === 0 && (
                <p className="text-[9px] text-orange-500 mt-2 font-black uppercase">Trebuie să creezi mai întâi un cont de tip "Backtest" în Accounts.</p>
            )}
          </div>

          <div className="grid grid-cols-2 gap-4">
              <div>
                <label className={labelClass}>Primary Symbol</label>
                <input 
                type="text" 
                required
                className={inputClass}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className={labelClass}>Historical Start</label>
              <input 
                type="date" 
                required
                className={inputClass}
            ...
            <div>
              <label className={labelClass}>Historical End</label>
              <input 
                type="date" 
                required
                className={inputClass}
          ...
          <div className="flex items-center justify-end space-x-3 pt-6 border-t border-slate-800">
            <button 
              type="button"
              onClick={onClose}
              className="px-6 py-3 text-[10px] font-black uppercase text-slate-500 hover:text-white"
            >
              Cancel
            </button>
            <button 
              ...
              className={`px-10 py-3 rounded-xl text-[10px] font-black uppercase transition-all shadow-xl active:scale-[0.98] ${!formData.accountId ? 'bg-slate-800 text-slate-600' : 'bg-indigo-600 hover:bg-indigo-500 text-white shadow-indigo-600/30'}`}
            >
              Initialize Lab
            </button>
          </div>
