...
import { useAppStore } from '../AppContext'; // FIX: Imported useAppStore
import { getRealtimeMarketContext, getDashboardInsights } from '../geminiService'; // FIX: Imported geminiService functions

interface AnalyticsProps {
  // language: Language; // `language` prop removed, now fetched from store
}

const Analytics: React.FC<AnalyticsProps> = () => {
  const trades = useAppStore(state => state.trades);
  const language = useAppStore(state => state.language);
  const weeklyPreps = useAppStore(state => state.weeklyPreps); // NEW: Get weeklyPreps from store
  const loadDailyPreps = useAppStore(state => state.loadDailyPreps); // NEW: Get lazy load function
  const loadWeeklyPreps = useAppStore(state => state.loadWeeklyPreps); // NEW: Get lazy load function
  ...
  const currentWeekContext = useMemo(() => {
    // Only proceed if weeklyPreps is loaded
  ...
  const fetchAIIntel = async (signal?: AbortSignal) => { // NEW: Accept signal
    // Disabled
  };
  ...
  useEffect(() => {
    loadDailyPreps(); 
    loadWeeklyPreps(); 
    // fetchAIIntel call removed to disable automatic fetch
