...
import { Playbook, PlaybookRule, PlaybookTag, Trade, TradeScreenshot } from '../types';
...
export default function PlaybookEditor({ playbooks = [], onSave, onDelete, language, trades }: PlaybookEditorProps) {
  const { id } = useParams();
    ...
    <div className="max-w-6xl mx-auto space-y-12 animate-in slide-in-from-bottom-4 duration-700 pb-20">
      <div className="flex items-center justify-between">
        ...
        <div className="flex space-x-4">
          <button onClick={collapseAll} className="text-[10px] font-black text-slate-500 hover:text-white uppercase tracking-widest px-4 border border-slate-800 rounded-lg">Collapse All</button>
          <button onClick={expandAll} className="text-[10px] font-black text-slate-500 hover:text-white uppercase tracking-widest px-4 border border-slate-800 rounded-lg">Expand All</button>
        ...
        <div className="lg:col-span-2 space-y-12">
          ...
          <div className="space-y-10">
            ...
            <div className="space-y-8">
              <div>
                ...
                <div className="space-y-6 mb-10">
                   ...
                   {tieredSuggestions[0].length > 0 && (
                       <div className="space-y-3">
                           <button 
                                onClick={() => toggleCollapseSuggestion(0)}
                                className="flex items-center space-x-2 group/header w-full text-left"
                           >
                               <i className="fas fa-tags text-[10px] text-slate-600"></i>
                               <p className="text-[9px] font-black text-slate-600 uppercase tracking-widest flex-1">Uncategorized / Legacy</p>
                               <i className={`fas fa-chevron-${collapsedSuggestions[0] ? 'down' : 'up'} text-[8px] text-slate-700 group-hover/header:text-slate-400 transition-all`}></i>
                           </button>
                           {!collapsedSuggestions[0] && (
                               <div className="flex flex-wrap gap-2 animate-in fade-in duration-200">
                                   {tieredSuggestions[0].map(tag => (
                                       <button
                                           key={tag.id}
                                           onClick={() => pickSuggestedTag(tag)}
                                           className="px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest border border-slate-800 hover:border-slate-600 transition-all flex items-center space-x-2 bg-slate-950 group/tagbtn"
                                       >
                                           <span className="text-slate-500 group-hover/tagbtn:text-white">{tag.text}</span>
                                           <i className="fas fa-plus text-[7px] text-slate-800 group-hover/tagbtn:text-slate-400"></i>
                                       </button>
                                   ))}
                               </div>
                           )}
                ...
                {/* Grouped Active Tags - SHRUNK BOXES AS REQUESTED */}
                <div className="bg-[#0f0f1b] border border-slate-800 rounded-[2rem] p-6 shadow-inner grid grid-cols-1 md:grid-cols-2 gap-6">
                           ...
                           <div key={tierDef.tier} className={`space-y-2 p-4 rounded-2xl border border-slate-800/50 bg-slate-900/20 transition-all duration-300 ${activeCount === 0 ? 'opacity-20' : 'opacity-100'} ${isCollapsed ? 'pb-2' : ''}`}>
                               ...
                               {!isCollapsed && (
                                   <div className="flex flex-wrap gap-2 pt-2 animate-in fade-in duration-300">
                                               ...
                                               <div className="flex gap-1" onClick={(e) => e.stopPropagation()}>
                                                   {TAG_COLORS.slice(0, 6).map(c => (
                                                       <button 
                                                           ...
                                                           className={`w-2.5 h-2.5 rounded-full transition-all hover:scale-125 border ${tag.color === c ? 'border-white' : 'border-transparent'}`}
                                       ...
                                       {tieredActiveTags[tierDef.tier].length === 0 && (
                                           <p className="text-[7px] text-slate-800 italic uppercase py-1">Undefined</p>
                                       )}
                                   </div>
                               )}
                           </div>
                   ...
                   {tieredActiveTags[0].length > 0 && (
                       <div className="space-y-2 p-4 rounded-2xl border border-slate-800/50 bg-slate-900/20 md:col-span-2">
                           <div 
                                className="flex items-center justify-between border-b border-slate-800/40 pb-2 cursor-pointer group/uc"
                                onClick={() => toggleCollapseActive(0)}
                           >
                               <div className="flex items-center space-x-2">
                                   <i className="fas fa-tags text-xs text-slate-500"></i>
                                   <h5 className="text-[9px] font-black text-white uppercase tracking-widest">Extra Attributes</h5>
                               </div>
                               <i className={`fas fa-chevron-${collapsedActive[0] ? 'down' : 'up'} text-[9px] text-slate-700 group-hover/uc:text-white transition-all`}></i>
                           </div>
                           {!collapsedActive[0] && (
                               <div className="flex flex-wrap gap-3 pt-2 animate-in fade-in duration-300">
                                   {tieredActiveTags[0].map(tag => (
                                       <div key={tag.id} className="flex flex-col space-y-2 bg-slate-900/80 border border-slate-800 p-3 rounded-xl group relative shadow-md hover:border-slate-600 transition-all min-w-[140px]">
                                           <button onClick={(e) => { e.stopPropagation(); removeTag(tag.id); }} className="absolute -top-1.5 -right-1.5 bg-red-600 text-white w-5 h-5 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all z-10 shadow-xl">
                                               <i className="fas fa-times text-[8px]"></i>
                                           </button>
                                           <input 
                                           ...
                                           <div className="flex gap-1" onClick={(e) => e.stopPropagation()}>
                                               {TAG_COLORS.slice(0, 6).map(c => (
                                                   <button key={c} onClick={() => updateTag(tag.id, { color: c })} className={`w-2.5 h-2.5 rounded-full border ${tag.color === c ? 'border-white' : 'border-transparent'}`} style={{ backgroundColor: c }} />
                                               ))}
                                           </div>
                                       </div>
                                   ))}
            ...
            <div>
              ...
              <HoverDialog label="Setup Description" text={formData.description}>
                <textarea 
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              <div>
                <label className={labelClass}>Entry At</label>
                <HoverDialog label="Entry Logic" text={formData.entryAt}>
                    <input type="text" value={formData.entryAt} onChange={(e) => setFormData({...formData, entryAt: e.target.value})} className={inputClass} placeholder="Ex: Minimum Pullback" />
                </HoverDialog>
              </div>
              <div>
                <label className={labelClass}>Target</label>
                <HoverDialog label="Profit Target" text={formData.target}>
                    <input type="text" value={formData.target} onChange={(e) => setFormData({...formData, target: e.target.value})} className={inputClass} placeholder="Ex: HTF targets" />
                </HoverDialog>
              </div>
              <div>
                <label className={labelClass}>Invalidation</label>
                <HoverDialog label="Invalidation Point" text={formData.invalidation}>
                    <input type="text" value={formData.invalidation} onChange={(e) => setFormData({...formData, invalidation: e.target.value})} className={inputClass} placeholder="Ex: Back in Opening Range" />
                </HoverDialog>
              </div>
            </div>
          </div>
              ...
              {(formData.screenshots || []).map((shot, idx) => (
                <div key={idx} className="bg-slate-900 border border-slate-800 rounded-[2.5rem] overflow-hidden shadow-2xl group relative transition-all hover:border-blue-500/30">
                  <div className="relative aspect-video bg-black flex items-center justify-center border-b border-slate-800 cursor-zoom-in" onClick={() => setLightboxImage(shot.url)}>
                    ...
                    <button onClick={(e) => { e.stopPropagation(); setFormData(prev => ({ ...prev, screenshots: prev.screenshots?.filter((_, i) => i !== idx) })); }} className="absolute top-6 right-6 bg-red-600/90 hover:bg-red-600 text-white w-10 h-10 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all z-20 shadow-xl">
                      <i className="fas fa-times"></i>
                    </button>
                  </div>
                  <div className="p-6 bg-slate-950/40">
                    <input type="text" value={shot.caption} onChange={(e) => setFormData(prev => ({ ...prev, screenshots: prev.screenshots?.map((s, i) => i === idx ? { ...s, caption: e.target.value } : s) }))} className="w-full bg-slate-900 border border-slate-800 rounded-xl px-5 py-3 text-xs text-slate-300 outline-none focus:ring-1 focus:ring-blue-500 italic transition-all" placeholder="Add technical observation about this example..." />
                  </div>
                </div>
              ))}
              {(formData.screenshots || []).length === 0 && (
                <div onClick={() => fileInputRef.current?.click()} className="col-span-full py-24 text-center border-2 border-dashed border-slate-800 rounded-[3rem] hover:border-blue-500/40 hover:bg-blue-500/[0.02] cursor-pointer transition-all shadow-inner">
                  <i className="fas fa-image text-5xl text-slate-800 mb-6"></i>
                  <p className="text-[11px] font-black text-slate-600 uppercase tracking-[0.4em]">Drag & drop or click to upload strategy examples</p>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="space-y-12">
          <div className="bg-[#0b1222] border border-slate-800 p-10 rounded-[3rem] shadow-2xl h-fit sticky top-24">
            <div className="flex items-center space-x-4 mb-10 border-b border-slate-800/50 pb-6">
              ...
              <div>
                <h4 className="text-sm font-black text-white uppercase tracking-tight">Trade History</h4>
                <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Recent Executions</p>
              </div>
            </div>
            <div className="space-y-6">
                ...
                {relatedTrades.map(trade => (
                  <Link to={`/trade/${trade.id}`} key={trade.id} className="group relative aspect-square bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden hover:border-blue-500/50 transition-all shadow-xl">
                    {trade.screenshots && trade.screenshots[0] ? (
                      <img src={trade.screenshots[0].url} className="w-full h-full object-cover group-hover:scale-125 transition-transform duration-700" alt="Trade Thumbnail" />
                    ) : (
                      <div className="w-full h-full flex flex-col items-center justify-center bg-slate-950/80">
                        <span className="text-[10px] font-black text-slate-700 uppercase mb-1 tracking-tighter">{trade.instrument}</span>
                        <i className="fas fa-chart-line text-slate-800 text-xl"></i>
                      </div>
                    )}
                    ...
                    <div className="absolute inset-0 bg-blue-600/30 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center z-10">
                      <div className="w-8 h-8 rounded-full bg-white text-blue-600 flex items-center justify-center shadow-2xl scale-0 group-hover:scale-100 transition-transform duration-300">
              ...
              {relatedTrades.length === 0 && (
                <div className="py-16 text-center bg-slate-950/30 border border-dashed border-slate-800 rounded-2xl">
                  <p className="text-[10px] text-slate-700 font-black italic uppercase tracking-widest">No past executions tracked</p>
                </div>
              )}
              {relatedTrades.length > 0 && (
                <p className="text-[9px] font-black text-slate-600 uppercase text-center mt-6 tracking-[0.2em]">Validated with {relatedTrades.length} samples</p>
              )}
            </div>
            
            <div className="mt-12 bg-blue-600/5 border border-blue-500/20 p-8 rounded-[2rem] flex flex-col space-y-5 relative overflow-hidden group/tip">
              <div className="w-12 h-12 rounded-xl bg-blue-600 flex items-center justify-center text-white shadow-lg relative z-10 group-hover/tip:scale-110 transition-transform"><i className="fas fa-lightbulb"></i></div>
              <div className="relative z-10">
                <h5 className="text-xs font-black text-white uppercase tracking-widest mb-2">Master your Setup</h5>
                <p className="text-[10px] text-slate-400 font-medium leading-relaxed italic">"The setups you skip are just as important as the ones you take. Upload examples of failures to build pattern recognition for high-risk scenarios."</p>
              </div>
              <i className="fas fa-brain absolute -bottom-8 -right-8 text-8xl text-white/5 opacity-40 group-hover/tip:rotate-12 transition-transform"></i>
            </div>
          </div>
        </div>
      </div>
      ...
      {lightboxImage && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/98 backdrop-blur-2xl p-6 animate-in fade-in duration-500 cursor-zoom-out" onClick={() => setLightboxImage(null)}>
          ...
          <div className="max-w-[98vw] max-h-[98vh] relative flex items-center justify-center">
            <img src={lightboxImage} className="max-w-full max-h-[92vh] object-contain shadow-[0_0_80px_rgba(0,0,0,0.8)] rounded-2xl select-none" alt="Enlarged Reference View" onClick={(e) => e.stopPropagation()} />
          </div>
        </div>
      )}
    </div>
