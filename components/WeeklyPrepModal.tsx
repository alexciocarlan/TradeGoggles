...
import { WeeklyPrepData, BiasType, TradeScreenshot, WeeklyNewsEvent, WeeklyOTF, WeekOpenType, WeeklyStructureAnomaly, WeeklyPOCDivergence } from '../types';
...
const WeeklyPrepModal: React.FC<WeeklyPrepModalProps> = ({ isOpen, onSave, onClose, initialData, selectedDate }) => {
  ...
  const [formData, setFormData] = useState<WeeklyPrepData>(initialData || {
    ...
    weeklyOTF: 'None', weekOpenType: 'None', weeklyStructureAnomaly: 'None', weeklyPocDivergence: 'None',
    matrixScore: 0, matrixRegime: 'UNDEFINED',
  ...
  // --- AMT LOGIC V4.5: VOLATILITY & CONTEXT MATRIX ENGINE ---
  const matrixCalculation = useMemo(() => {
    let score = 0;
    let hasHighImpactNews = false;

    // 0. Detectie High Impact News (Secțiunea 6.3)
    if (formData.dayNews) {
        // FIX: Cast dayEvents as WeeklyNewsEvent[] to resolve Property 'some' does not exist on type 'unknown' error.
        Object.values(formData.dayNews).forEach((dayEvents: WeeklyNewsEvent[]) => {
            if (dayEvents.some(e => e.tier === 3)) hasHighImpactNews = true;
        });
    }
    
    // 1. Starea Trendului Saptamanal (WT)
    if (formData.weeklyOTF === 'OTF Up') score += 2;
    if (formData.weeklyOTF === 'OTF Down') score -= 2;

    // 2. Relatia Valorii (VR)
    if (formData.priceVsPWeek?.includes('outside') && formData.pwClosePosition?.includes('Upper')) score += 1; 
    if (formData.priceVsPWeek?.includes('outside') && formData.pwClosePosition?.includes('Lower')) score -= 1; 

    // 3. Tipul Deschiderii (OC)
    if (formData.weekOpenType === 'Gap Up') score += 2;
    if (formData.weekOpenType === 'Gap Down') score -= 2;

    // 4. Tinte Structurale (AT)
    if (formData.weeklyStructureAnomaly === 'Poor High') score += 1; 
    if (formData.weeklyStructureAnomaly === 'Poor Low') score -= 1; 

    // 5. POC Divergence (PD)
    if (formData.weeklyPocDivergence === 'Divergent (Bullish)') score += 1;
    if (formData.weeklyPocDivergence === 'Divergent (Bearish)') score -= 1;

    let regime = 'BALANCE / ROTATION';
    if (score >= 4) regime = 'STRONG TREND UP';
    if (score <= -4) regime = 'STRONG TREND DOWN';
    if (score >= 2 && score < 4) regime = 'MODERATE BULLISH';
    if (score <= -2 && score > -4) regime = 'MODERATE BEARISH';

    // Ajustare regim pentru Volatilitate (Secțiunea 6.3)
    if (hasHighImpactNews) {
        regime += " (POROUS VALUE / HI-VOL)";
    }

    return { score, regime, hasHighImpactNews };
  }, [
    formData.weeklyOTF, 
    formData.priceVsPWeek, 
    formData.pwClosePosition, 
    formData.weekOpenType, 
    formData.weeklyStructureAnomaly, 
    formData.weeklyBias,
    formData.weeklyPocDivergence,
    formData.dayNews
  ]);

  useEffect(() => {
      if (formData.matrixScore !== matrixCalculation.score || formData.matrixRegime !== matrixCalculation.regime) {
          setFormData(prev => ({ 
              ...prev, 
  ...
  }, [matrixCalculation.score, matrixCalculation.regime]);
      ...
      <div className="bg-[#060b13] border border-slate-800 rounded-[3rem] w-full max-w-[1050px] max-h-[96vh] overflow-hidden flex flex-col shadow-[0_0_120px_rgba(0,0,0,0.8)] animate-in zoom-in-95 duration-300">
        ...
        <div className="px-10 py-8 border-b border-slate-800/40 flex justify-between items-center bg-[#060b13] shrink-0">
          <div className="flex items-center space-x-5">
            ...
            <div>
              <h2 className="text-2xl font-black text-white uppercase italic tracking-tighter leading-none mb-1">WEEKLY ANCHOR: MACRO CALIBRATION</h2>
        ...
        <div className="p-10 overflow-y-auto space-y-12 custom-scrollbar bg-[#03070c] flex-1">
          ...
          <section>
             <div className="flex justify-between items-center mb-8">
                ...
                {matrixCalculation.hasHighImpactNews && (
          ...
          {/* STEP 1: CONTEXT MATRIX & BIAS */}
          <section>
             <h4 className={sectionTitleClass}><span className="w-5 h-5 rounded-md bg-blue-500/10 text-blue-500 flex items-center justify-center mr-3 text-[9px]">1</span> CONTEXT SCORING MATRIX</h4>
             <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                
                <div className="lg:col-span-8 bg-slate-900/10 border border-slate-800/60 p-8 rounded-[2.5rem] space-y-8">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label className={labelClass}>WEEKLY ONE-TIMEFRAMING (OTF)</label>
                        ...
                        <div>
                            <label className={labelClass}>WEEK OPEN TYPE</label>
                        ...
                        <div>
                            <label className={labelClass}>POC DIVERGENCE (TPO vs VOL)</label>
                            <select value={formData.weeklyPocDivergence} onChange={e => setFormData({...formData, weeklyPocDivergence: e.target.value as any})} className={inputClass}>
                                <option value="None">None</option>
                                <option value="Convergent">Convergent</option>
                                <option value="Divergent (Bearish)">Divergent (Bearish - High Prices Rejected)</option>
                                <option value="Divergent (Bullish)">Divergent (Bullish - Low Prices Rejected)</option>
                            </select>
                        </div>
                        <div>
                            <label className={labelClass}>UNFINISHED BUSINESS (MAGNETS)</label>
                            <select value={formData.weeklyStructureAnomaly} onChange={e => setFormData({...formData, weeklyStructureAnomaly: e.target.value as any})} className={inputClass}>
                                <option value="None">None</option>
                                <option value="Poor High">Poor High (Bullish Magnet)</option>
                                <option value="Poor Low">Poor Low (Bearish Magnet)</option>
                    ...
                    <div className="pt-6 border-t border-slate-800/50">
                        <p className="text-[9px] font-black text-slate-500 uppercase mb-4 tracking-[0.2em]">VALUE & BIAS INPUTS</p>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div>
                                ...
                                <select value={formData.priceVsPWeek} onChange={e => setFormData({...formData, priceVsPWeek: e.target.value})} className={inputClass}>
                                    <option value="None">None</option>
                                    <option value="inside pwRange">inside pwRange</option>
                                    <option value="outside pwRange">outside pwRange</option>
                                    <option value="inside pwVA">inside pwVA</option>
                                    <option value="outside pwVA">outside pwVA</option>
                                </select>
                            </div>
                            <div>
                                <label className={labelClass}>PW CLOSE POSITION</label>
                                <select value={formData.pwClosePosition || 'None'} onChange={e => setFormData({...formData, pwClosePosition: e.target.value})} className={inputClass}>
                                    <option value="None">None</option>
                                    <option value="Upper Extreme Quartile">Upper Extreme Quartile</option>
                                    <option value="Upper Middle Quartile">Upper Middle Quartile</option>
                                    <option value="Lower Middle Quartile">Lower Middle Quartile</option>
                                    <option value="Lower Extreme Quartile">Lower Extreme Quartile</option>
                                </select>
                            </div>
                        </div>
                    </div>
                ...
                <div className="lg:col-span-4 flex flex-col gap-6">
                    <div className={`bg-[#0b1222] border p-8 rounded-[2.5rem] shadow-xl text-center relative overflow-hidden flex-1 flex flex-col justify-center transition-all duration-500 ${matrixCalculation.hasHighImpactNews ? 'border-orange-500/40' : 'border-slate-800'}`}>
                        <div className="relative z-10">
                            <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">MATRIX CONTEXT SCORE</p>
                    ...
                    <div className="bg-slate-900/10 border border-slate-800/60 p-6 rounded-[2rem] space-y-4 shadow-inner">
                         <div className="flex items-center space-x-2 mb-2">
                             ...
                             <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Protocol Guidance (6.3)</p>
                         </div>
                         <p className="text-[10px] text-slate-400 font-medium italic leading-relaxed">
                            {matrixCalculation.hasHighImpactNews 
                                ? "Evenimentele Tier 3 detected. Value Area devine 'poroasă'. Lărgește marjele de eroare cu 50% la strategii 'Fade'."
                                : "Volatilitate macro standard. Nivelele structurale au precizie nominală."}
                         </p>
                    </div>
                </div>
             </div>
