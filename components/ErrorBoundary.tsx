...
export default class ErrorBoundary extends Component<Props, State> {
  ...
  public componentDidCatch(error: Error, errorInfo: ErrorInfo) {
    console.error("Critical Protocol Failure Trace:", error, errorInfo);
  }

  private handleReset = () => {
    if (window.confirm("Această acțiune va încerca să reseteze stocarea locală pentru a repara aplicația. Continui?")) {
      try {
        // Folosim try-catch pentru a preveni prăbușirea în medii securizate
        localStorage.clear();
          ...
          <div className="bg-[#0b1222] border border-red-500/30 rounded-[3rem] p-12 max-w-2xl w-full shadow-2xl relative overflow-hidden">
            <div className="relative z-10 text-center">
              ...
              <p className="text-slate-400 text-sm leading-relaxed mb-10 font-medium">
                Sistemul a întâmpinat o eroare critică. Acest lucru se datorează adesea restricțiilor de securitate ale browserului în mod Incognito sau într-un sandbox.
              </p>
              
              <div className="bg-black/40 border border-slate-800 p-6 rounded-2xl mb-10 text-left">
