...
const MarketHubBar: React.FC<MarketHubBarProps> = ({ 
  ...
  // OPTIMIZED SELECTOR: Use useShallow
  const { 
    trades, accounts, selectedAccountId, setSelectedAccountId, 
    riskManagerEnabled, language, isOfflineMode
  ...
  // CIRCUIT BREAKER DISABLED: Always allow trading regardless of risk/protocol status
    ...
    <div className="w-full bg-[#060b13] border-b border-slate-800/50 px-6 py-3 flex items-center justify-between no-print overflow-x-auto whitespace-nowrap custom-scrollbar transition-colors duration-300 sticky top-0 z-[100]">
      <div className="flex items-center space-x-6">
        <div className="border-r border-slate-800/50 pr-6">
            ...
            >
                <i className={`fas ${isLocked ? 'fa-lock' : 'fa-plus'} text-[10px]`}></i>
                <span className="text-[10px] uppercase tracking-[0.15em] font-black">{isLocked ? 'LOCKED' : 'TRADE'}</span>
            </button>
        </div>
        ...
        <div className="border-r border-slate-800/50 pr-6">
          <div className={`flex items-center space-x-2 px-3 py-1.5 rounded-lg border ${
            ...
            <span className="text-[9px] font-black uppercase tracking-widest">
              {isOfflineMode ? 'OFFLINE MODE' : 'LOCAL MODE'}
            </span>
          </div>
        </div>
